...
for key, default in [("coin", 0), ("inventory", []), ("shop_open", False), ("items", {})]:
    if key not in st.session_state or (key == "items" and not isinstance(st.session_state.items, dict)):
...
with col3:
    st.subheader("🏪 상점")
    st.session_state.shop_open = st.checkbox("상점 열기", value=st.session_state.shop_open, key="shop_open_checkbox")
