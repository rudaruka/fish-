...
def reset_game_data():
    """st.button의 on_click에 연결될, 안전한 게임 초기화 콜백 함수."""
    
    # 🚨 핵심: 상태만 변경하고 성공 메시지를 띄웁니다.
    for key, value in DEFAULT_STATE.items():
        st.session_state[key] = value
        
    st.session_state.fishbook = set()
    
    st.success("게임 데이터가 완전히 초기화되었습니다. 페이지를 새로고침하거나 낚시를 진행하세요.")
...
# 🚨 최종 안전 초기화 버튼: on_click에 콜백 함수 연결
if st.button(
    "🚨 모든 데이터 초기화 (되돌릴 수 없음)", 
    type="danger", 
    key="reset_game_btn_final",
    on_click=reset_game_data  # 안전하게 상태를 초기화하는 함수 호출
):
    pass
