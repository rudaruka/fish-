...
if st.session_state.shop_open:
    st.subheader("🏪 상점")
    if st.session_state.inventory:
        selected = st.multiselect("판매할 물고기 선택 (다중 선택 가능)", st.session_state.inventory)
        if st.button("판매하기"):
            total_price = 0
            for f in selected:
